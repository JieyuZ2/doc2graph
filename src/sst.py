...
class SST(data.Dataset):
    ...
    def __init__(self, path, text_field, label_field, mask_field, subtrees=False,
        ...
        with open(os.path.expanduser(path)) as f:
            if subtrees:
                examples = [ex for line in f for ex in
                            self.fromtree(line, fields, True)]
            else:
                examples = [self.fromtree(line, fields) for line in f]
        super(SST, self).__init__(examples, fields, **kwargs)

    @classmethod
    def splits(cls, text_field, label_field, mask_field, root='.data',
        ...
        train_data = None if train is None else cls(
            os.path.join(path, train), text_field, label_field, mask_field, subtrees=train_subtrees,
            **kwargs)
        val_data = None if validation is None else cls(
            os.path.join(path, validation), text_field, label_field, mask_field, **kwargs)
        test_data = None if test is None else cls(
            os.path.join(path, test), text_field, label_field, mask_field, **kwargs)
    ...
    @classmethod
    def fromtree(cls, datas, fields, subtrees=False):
        try:
            ...
            raise
        tree = Tree.fromstring(datas)
        if subtrees:
            return [data.Example.fromlist(
                [' '.join(t.leaves()), t.label()], fields) for t in tree.subtrees()]
        return data.Example.fromlist([' '.join(tree.leaves()), tree.label(), ' '.join(tree.leaves())], fields)